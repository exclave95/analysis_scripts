...
with open("SP_results.txt", "w") as file:
    ...
    # file.write(f'\nReference: {ref}\nFull selection: ')
    file.write(f'\nDynamic: {dynamic}\nGeometry: from {z0} to {dz} Ångstrom(s)\nframes: {frame_start} to {frame_stop}\ntau: {taumax}')
    if curvefit == "ak":
...
# layers to study
start_top_z = clay_max_z + z0
end_top_z = start_top_z + dz

start_bottom_z = clay_min_z - z0
end_bottom_z = start_bottom_z - dz    


dynamic_resids = u.select_atoms(f'{dynamic}').resids
...
#select = f"{dynamic} and (prop z >={start_top_z} and "f"prop z <= {end_top_z} and "f"prop z <={start_bottom_z} and "f"prop z >= {end_bottom_z} )"
if side == 'top':
    select = f"{dynamic} and (prop z >={start_top_z} and "f"prop z <= {end_top_z})"
elif side == 'bottom':
    select = f"{dynamic} and (prop z <={start_bottom_z} and "f"prop z >= {end_bottom_z})"
...
if csv == 'yes':
    ...
    # save as a csv file 
    np.savetxt(f'{csv_filename}.csv', surv_prob_data, delimiter = ',', header=f'SP timeseries of sel {dynamic}, {z0} to {dz}, count {counter}\n{cwd}')  
...
try:
    ...
    with open('SP_results.txt', 'a') as file:
        ...
        file.write(f'\nCurve fit parameters for {dynamic}, {z0} to {z0+dz})')
...
ax1.set_title(f'SP - {dynamic} within {z0} to {z0+dz}')
